...
SOMEDELAY             EQU 2750000      ; a faster than 500ms delay given the ~16 MHz clock
...
; Init the Ports for use - this has the side effect of turning off all LEDs 
		...
		MOV R3, #0x00				; load in the value to turn the RED led OFF, RED LED is bit 1 (counting from 0)
...
Shift_and_display
		...
		;next line wasn't originally there... above was originally LSLS
		BLCC 	    LED_OFF		; go to FLAG if the CARRY bit is 0 ;Originally BEQ
		BLCS		LED_ON		; go to LED_ON if the CARRY bit is 1 ;Originally BNE
		
		;BCC	label1	; branch somewhere it's zero because the Carry flag is clear
		;BCS	label2	; branch somewhere - it's not zero because the Carry flag is set
...
;  init R0 before calling DELAY to set how long to delay
		BL			DELAY                   ; **allow delay before the other instruction
		
		TEQ		R1, #0x00
		BEQ		ENDOFLETTER	 ;if R8 is 0, go to the next char ;if R1 is 0 go to next
		BNE		Shift_and_display ;if R8 is not 0, shift to the next bit in the binary morse code; if R1 is not 0

		;BL	DELAY ;This was inserted into LED_ON and LED_OFF
		;UPDATE: THIS WAS INSERTED AS A SEPARATE SUBROUTINE
...
;  Now test R1 for zero to see if all bits are shifted as the last bit is always a 1 and it's followed by 0'sA
	;test R1 for 0 to see if we're done
		; OR WE COULD HAVE A COUNTER CHECKING THE NUMBER OF BITS REMAINING
		
;  If we're not done then loop back to Shift_and_display to shift the next bit
		;This is inserted into LED_ON and LED_OFF
...
CHAR2MORSE	STMFD		R13!,{R14}	; push Link Register (return address) on stack
		...
		;
		SUB			R0, #0x41 ; Subtract 41 from ASCII value of char
		
		MOV			R12, #0x02 ; increment by 2 bytes
		MUL			R0, R0, R12 ; calculate the offset from the address
		
		LDR			R10, =MorseLUT ; read the morse patterm from the MorseLUT
		ADD			R10, R0 ; Adds the offset to the MorseLUT address
		
		LDRH		R1, [R10]; Loads the Morse Binary Code with the contents of R10 (address of MorseLUT+offset)
...
LED_ON 	   	STMFD		R13!,{R3, R4}		; preserve R3 and R4 on the stacked pointed to by R13
		...
		MOV			R3, #0x02               ; **put the value for RED LED into R3
		STR			R3, [R4]                ; **store contents of R1 into R3 to turn LED on
		MOV			R0, #0x1
...
LED_OFF	   	STMFD		R13!,{R3, R14}	; push R3 and Link Register (return address) on stack
		;... insert your code here
		MOV			R3, #0x00			; **put #0x00 into R3 to turn the LED off
		STR			R3, [R4]            ; **store contents of R1 into R3 to turn the LED off
		MOV			R0, #0x1
...
MultipleDelay		TEQ		R0, #0		; test R0 to see if it's 0 and if so exit - this sets the Zero flag so you can use BEQ, BNE
			;... insert your code here
					LDR R10, =SOMEDELAY   	       ; R0 is initialized to a value to give a 500ms delay

loop1
    SUBS 	R10, R10, #1                 ; R0 = R0 - 1 (count = count - 1) and set N, Z, C status bits
	; Note: For SUBs the "s" suffix means to set the status bits, without this the loops would not exit
	BNE 	loop1 ; If the R10/counter is not 0, keep decreasing
	SUBS 	R0, #1 ; Decrement the value of R0 (loop counter)
	BEQ 	exitDelay ; If R0 is 0, we don't have to loop again
	BNE		MultipleDelay ; If R0 is NOT 0, we need to go back to loop1
...
InputLUT	DCB		"VBNSA", 0	; strings must be stored, and read, as BYTES
                                ; Read the bytes of the string until the "0" is reached
		ALIGN				; make sure things fall on word addresses
MorseLUT 
