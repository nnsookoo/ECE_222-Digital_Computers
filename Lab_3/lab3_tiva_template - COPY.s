...
; The last LED is on port E - so read the value, modify bit 1 as it drives the LED and write it back out - this is called read-modify-write
	...
	BL 	RandomNum ; ADDED - Branch to RandomNum to generate a number to multiply the 0.1ms delay by
	MOV R1, R11; ADDED - Store the random number in R1 to multiply the delay by in order to generate a random delay
	BL RandomMap; Added 
	BL 	ShortDelay ; ADDED - We need to branch to the delay
...
; The value of this counter will later be displayed after the user has pushed the button
	...
	MOV	R5, #0 ;Initialize a display counter
	MOV R1 , #1 ;Only one delay loop each time
	BL POLLING_LOOP
...
POLLING_LOOP
	STMFD		R13!,{R0, R3, R4, R14}		; pull the LR or return address and return

POLLING
	BL ShortDelay; ShortDelay subroutine - delay for a fixed amount of time. Go for a 0.1ms delay loop
	ADD R5, R5, #1 ;increment polling counter
	
	MOV R3, #SW1
	LDR R4, =GPIO_PORTF_DATA_R ; pointer to Port F data register
	LDR R0, [R4, #GPIO_DATA_OFFSET]  ;added - *naveela*; unsure             
		
    ;LDR  R0, [R4]	; load in the value to turn the RED led OFF, RED LED is bit 1 (counting from 0)
	
	AND R3, R0               ; Reads switch
	CMP     R3, #0
	BEQ		Bitshift
    BNE     POLLING
	LDMFD		R13!,{R0,R3,R4, R15}		; pull the LR or return address and return
...
RandomMap
CONSTANT             EQU 20000      ;
	STMFD		R13!,{R0, R14}		; push the LR or return address
	LSR R1, R1 , #3  ; divide by 8
	LDR R0,=CONSTANT
	ADD R1, R1, R0; add 160000 to map in range
	LDMFD		R13!,{R0, R15}		; pull the LR or return address and return
...
Displayloop
	...
	LDR R3, =GPIO_PORTB + (PORT_B_MASK << 2) ; generate the base address for port B
	STR R4, [R3, #GPIO_DATA_OFFSET] ; GPIO_DATA_OFFSET is the address offset to get to the DATA part of the port

; The last LED is on port E - so read the value, modify bit 1 as it drives the LED and write it back out - this is called read-modify-write
	LDR R3, =GPIO_PORTE + (PORT_E_MASK << 2) ; generate the base address for port E
	LDR R1, [R3, #GPIO_DATA_OFFSET] ; GPIO_DATA_OFFSET is the address offset to get to the DATA part of the port
	AND R1, #0xFD ; set E1 0 to turn that LED off
	ORR R1, R4		; set B1 - these two lines could be replaced with a single ORR statement
	STR R1, [R3, #GPIO_DATA_OFFSET] ; write the data back out
