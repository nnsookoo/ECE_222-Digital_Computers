...
loop
	MOV R11, #0xf3c0 ; initialize the Random Number routine - NEVER WRITE TO R11 AFTER THIS
	BL 	RandomNum ; Branch to RandomNum to generate a number to multiply the 0.1ms delay by
	MOV R1, R11; Store the random number in R1 to multiply the delay by in order to generate a random delay
	BL RandomMap; Branch to RandomMap to scale the number
	BL 	ShortDelay ; We need to branch to the delay
	
; Turn on a LED after the delay and initialize a counter. 
; The value of this counter will later be displayed after the user has pushed the button
	...
	MOV	R5, #0 ;Initialize a counter R5 to counter how long a button push was taken to do
	
POLLING
	MOV R9, #4 ; Initialize the display counter R9. 32 bits to display to 4 sets of numbers
	MOV	R1, #1 ; Initialize R1 to provide one delay loop
	
	BL ShortDelay; ShortDelay subroutine - delay for a fixed amount of time. Go for a 0.1ms delay loop
	
	ADD R5, R5, #1 ;increment polling counter by one
	MOV R8, R5 ;Put counter into R8 for future use
	
	MOV R10, #SW1
	LDR R4, =GPIO_PORTF_DATA_R ; pointer to Port F data register
	LDR R10, [R4, #GPIO_DATA_OFFSET]  ;R10 will now look like 0x1040025000       
	
	LSR R10, #10
	BFI R7, R10, #0, #1 ; right shift r10 by 10 bits to check the input
	;ANDS R7, R7, #1
	TEQ R7, #1 ;pushed button
	BEQ DISPLAY_2
	BNE POLLING

DISPLAY_1
	MOV R1, #0x4E20 ; For a 2s delay
	BL ShortDelay
	MOV R8, R5
	MOV R9, #4
...
Displayloop
	;MOV        R0,#0xFF
	;AND        R0, R5		; Read a (byte) character to convert to a Morse pattern
	;turn on  port B LEDs on the shielf - the last LED is on Port E
	LDR R3, =GPIO_PORTB + (PORT_B_MASK << 2) ; generate the base address for port B
	STR R6, [R3, #GPIO_DATA_OFFSET] ; GPIO_DATA_OFFSET is the address offset to get to the DATA part of the port

; The last LED is on port E - so read the value, modify bit 1 as it drives the LED and write it back out - this is called read-modify-write
	LDR R3, =GPIO_PORTE + (PORT_E_MASK << 2) ; generate the base address for port E
	LDR R6, [R3, #GPIO_DATA_OFFSET] ; GPIO_DATA_OFFSET is the address offset to get to the DATA part of the port
	AND R6, #0xFD ; set E1 0 to turn that LED off
	ORR R6, R4		; set B1 - these two lines could be replaced with a single ORR statement
	STR R6, [R3, #GPIO_DATA_OFFSET] ; write the data back out
...
CONSTANT             EQU 20000      
	...
	LDR R0,=CONSTANT
	ADD R1, R1, R0; add 160000 to map in range
	LDMFD		R13!,{R0, R1, R15}		; pull the LR or return address and return
